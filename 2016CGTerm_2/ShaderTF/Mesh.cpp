...
{
	m_nNumberOfVertex = another.m_nNumberOfVertex;
	m_nNumberOfFace = another.m_nNumberOfFace;
	m_nNumberOfEdge = another.m_nNumberOfEdge;
	
	m_fCenterOfMass = another.m_fCenterOfMass;
	m_fDistancetoCOM = another.m_fDistancetoCOM;
	m_fRotateAngle = another.m_fRotateAngle;

	m_bIsTextureCoord = another.m_bIsTextureCoord;
	m_bIsVertexNormal = another.m_bIsVertexNormal;
	m_bUseMtlLib = another.m_bUseMtlLib;
	m_nMinFaceIdx = another.m_nMinFaceIdx;

	mVLists.insert(mVLists.begin(),another.mVLists.cbegin(), another.mVLists.cend());
	mFaces.insert(mFaces.begin(), another.mFaces.cbegin(), another.mFaces.cend());
	mEdges.insert(mEdges.begin(),another.mEdges.cbegin(), another.mEdges.cend());
	mTexCoordinates.insert(mTexCoordinates.begin(),another.mTexCoordinates.cbegin(), another.mTexCoordinates.cend());
	m_MtlLib.insert(m_MtlLib.begin(),another.m_MtlLib.cbegin(), another.m_MtlLib.cend());
}
...
bool Mesh::LoadMeshFromFile(char* filename, char* mtlFile, bool flag_material)
{
		...
		else 
		{
			while(!fs.eof())
			{
				...
				if( n == "#" )
				{
					...
					if(LoadMaterials(mtlFile)){
						m_MtlLib[0].name = name;
						continue;
					}
					else{
				...
				//	u = 1.0f - u;
					...
					if(end == 0) {
						m_MtlLib[mtlcnt].faceindex = glm::vec2(0.0,0.0);
						end ++;
						continue;
					}
					else {
						end = mFaces.size();
						m_MtlLib[mtlcnt].faceindex = glm::vec2(start,end);					
						start = end+1;
					}
					mtlcnt ++;
...
static GLuint* LoadTexImage(char* texFile, GLuint* texture){
...
bool Mesh::LoadMaterials(char* filename)
{
	...
	else 
	{
		...
		while(!fs.eof())
		{
			...
			if( n == "#" )
			{
				...
				index = (int)m_MtlLib.size();
				fs >> mtl_name;
				Material m;
				m.name = mtl_name;
				m_MtlLib.push_back(m);
			}
			else if ( n == "Ns") {
				float ns;
				fs >> ns;
				m_MtlLib[index].Ns = ns;
			}
			else if ( n == "Ni") {
				float ni;
				fs >> ni;
				m_MtlLib[index].Ni = ni;
			}
			else if ( n == "d") {
				float d;
				fs >> d;
				...
				fs >> y;
				fs >> z;
				m_MtlLib[index].Tf = glm::vec3(x,y,z);
			}
			else if ( n == "illum") {
				float il;
				fs >> il;
				m_MtlLib[index].illum = il;
			}
			else if ( n == "Ka") {
				float x, y, z;
				fs >> x;
				fs >> y;
				fs >> z;
				m_MtlLib[index].Ka = glm::vec3(x,y,z);
			}
			else if ( n == "Kd") {
				float x, y, z;
				fs >> x;
				fs >> y;
				fs >> z;
				m_MtlLib[index].Kd = glm::vec3(x,y,z);
			}
			else if ( n == "Ks") {
				float x, y, z;
				fs >> x;
				fs >> y;
				fs >> z;
				m_MtlLib[index].Ks = glm::vec3(x,y,z);
			}
			else if ( n == "Ke") {
				float x, y, z;
				fs >> x;
				fs >> y;
				fs >> z;
				m_MtlLib[index].Ke = glm::vec3(x,y,z);
			}
			else if (n == "map_Ka"){
				string fn = ".\\Meshes\\";
				string map_ka;
				...
				char *texfile = new char[fn.length() + 1];
				strcpy(texfile, fn.c_str());
		...
		//Print Log
	//	PrintMaterialInformation(filename);
	}
...
void Mesh::RenderTexMeshWithMaterials(int index){

	int start, end;
			
	start = m_MtlLib[index].faceindex.x;
	end = m_MtlLib[index].faceindex.y;
	
//	printf("Material[%d].face : %d, %d\n",index, start, end);

	glBegin(GL_TRIANGLES);

	for(int j = start; j < end; j++){
		int idx1, idx2, idx3;
		idx1 = mFaces[j].m_nVertexIdx.x;
		idx2 = mFaces[j].m_nVertexIdx.y;
		idx3 = mFaces[j].m_nVertexIdx.z;

		int tex1, tex2, tex3;
		tex1 = mFaces[j].m_nTexCoordIdx.x;
		tex2 = mFaces[j].m_nTexCoordIdx.y;
		tex3 = mFaces[j].m_nTexCoordIdx.z;
		...
		glVertex3f(	mVLists[idx1].m_fVertexPosition.x + m_fCenterOfMass.x, 
					mVLists[idx1].m_fVertexPosition.y + m_fCenterOfMass.y,
					mVLists[idx1].m_fVertexPosition.z + m_fCenterOfMass.z);	


		vNormal = mVLists[idx2].m_fVertexNormal;
		glNormal3f( vNormal.x, vNormal.y, vNormal.z); 
		glTexCoord2f(mTexCoordinates[tex2].x, mTexCoordinates[tex2].y);
		glVertex3f(	mVLists[idx2].m_fVertexPosition.x + m_fCenterOfMass.x, 
			mVLists[idx2].m_fVertexPosition.y + m_fCenterOfMass.y,
			mVLists[idx2].m_fVertexPosition.z + m_fCenterOfMass.z);	


		vNormal = mVLists[idx3].m_fVertexNormal;
		glNormal3f( vNormal.x, vNormal.y, vNormal.z); 
		glTexCoord2f(mTexCoordinates[tex3].x, mTexCoordinates[tex3].y);
		glVertex3f(	mVLists[idx3].m_fVertexPosition.x + m_fCenterOfMass.x, 
					mVLists[idx3].m_fVertexPosition.y + m_fCenterOfMass.y,
					mVLists[idx3].m_fVertexPosition.z + m_fCenterOfMass.z);	
	}
...
{
	if(m_bUseMtlLib){
		for (int i = 0; i < (int)m_MtlLib.size(); i++){
			
			RenderTexMeshWithMaterials(i);
		}
	}
	else{
		glBegin(GL_TRIANGLES);
